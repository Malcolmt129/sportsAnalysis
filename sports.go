...
//Get the stats of a particular player

func getNFLPlayerStats(env *Environment, playerID int, year int) {
	url := fmt.Sprintf("https://nfl-api-data.p.rapidapi.com/nfl-ath-statistics?id=%d&year=%d", playerID, year)
	...
	//Save the response body to a file
	file, err := os.Create("output/player.json")
	if err != nil {
		log.Println("Error creating file: ", err)
		return
	}
